...
This is the final chapter in the planned eighteen-chapter sequence, so there is no subsequent chapter cross-reference. Before treating the capstone as a service, revisit #section-ref(<sec-generations>, title: "policy generations") and #section-ref(<ch18-sec-portability>, title: "portability and operational caveats"), then turn the acceptance criteria into a kernel/configuration/architecture test matrix. The next technical action is not another hook: it is to prove the existing narrow contract under restart, telemetry loss, hard-link, rename, unsupported-filesystem, detach, and rollback tests.
